...
export class App {
  ...
  private createPanels(): void {
    ...
    // Wireless & Pentest Panels
    const signalIntelPanel = new SignalIntelPanel('signal-intel', t('panels.signalIntel') || 'Signal Intelligence');
    this.panels['signal-intel'] = signalIntelPanel;

    const relationshipGraph = new RelationshipGraph('relationship-graph', t('panels.relationshipGraph') || 'Entity Relationships');
    this.panels['relationship-graph'] = relationshipGraph;

    const pentestPanel = new PentestPanel('pentest-monitor', t('panels.pentestMonitor') || 'Autonomous Pentest');
    this.panels['pentest-monitor'] = pentestPanel;
  ...
  private async loadAllData(): Promise<void> {
    ...
    const tasks: Array<{ name: string; task: Promise<void> }> = [
      ...
      { name: 'wireless', task: runGuarded('wireless', () => this.loadWirelessSignals()) },
      { name: 'pentest', task: runGuarded('pentest', () => this.loadPentestStatus()) },
    ];
  ...
  private async loadWirelessSignals(): Promise<void> {
    if (SITE_VARIANT !== 'wireless' && !this.panelSettings['signal-intel']?.enabled) return;
  ...
  private setupRefreshIntervals(): void {
    ...
    this.scheduleRefresh('wireless', () => this.loadWirelessSignals(), 60 * 1000, () => SITE_VARIANT === 'wireless' || !!this.panelSettings['signal-intel']?.enabled);
    this.scheduleRefresh('pentest', () => this.loadPentestStatus(), 30 * 1000, () => SITE_VARIANT === 'pentest' || !!this.panelSettings['pentest-monitor']?.enabled);
  }
}
